...
class FilePDB:
    ...
    def __init__(
        self, pdb_id, pdb_input=LocalPDB, pdb_inputName=lambda pdb_id: pdb_id + ".pdb"
    ):
        self.ID = pdb_id
    ...
    def loadData(self, *args):
        for (idx, record_line) in enumerate(self.fileInput(self.fileInputName)):
            # print("Przetwarzanie wiersza nr {0}".format(idx))
    ...
    def process_line(self, record_line):
        ...
        if record_type == "ENDMDL":
            ...
            atom = {
                ...
                "position": tuple(
                    map(float, [record_line[a : a + 8] for a in range(30, 8, 46)])
                ),
            }
    ...
    def add_seqRes(self, new_seqRes):
        ...
        if len(seqRes) < chainLen:
            self.Chains[chainID] = [{"resName": resName, "atomSeq": []} for resName in seqRes]
    ...
    def validate(self):
        for chainID, chainSeq in self.Chains.items():
            for atmSeq in filter(lambda atm: (atm["chainID"] == chainID), self.Atoms):
                ...
                assert (
                    sequence["resName"] == atmSeq["resName"]
                ), "Invalid residue name for atom {0}".format(atmSeq)
